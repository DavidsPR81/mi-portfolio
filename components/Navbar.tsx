...
export default function Navbar({ theme, onThemeChange }: NavbarProps) {
  ...
  return (
    <>
      ...
      {/* Menú móvil mejorado */}
      {/* MENÚ MÓVIL CON SCROLL HORIZONTAL */}
      {mobileMenuOpen && (
        <div ref={mobileMenuRef} className="md:hidden bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 shadow-lg">
          {/* Navegación horizontal con scroll */}
          <div className="px-4 py-3">
            <div className="mobile-horizontal-scroll flex gap-3 pb-2">
              {navLinks.map(({ label, href }) => (
                <a
                  key={href}
                  href={href}
                  onClick={() => setMobileMenuOpen(false)}
                  className={`mobile-nav-item px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                    activeHash === href 
                      ? 'bg-teal-600 text-white shadow-md' 
                      : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-teal-50 dark:hover:bg-teal-900/20 hover:text-teal-600 dark:hover:text-teal-400'
                  }`}
                >
                  {label}
                </a>
